...
class CardController extends Controller
{
    ...
    public function moveSection(Request $request, Card $card): RedirectResponse
    {
        ...
        $targetSection = Section::query()->findOrFail($validated['sectionId']);
        $oldSectionId = $card->section_id;

        if ((int) $oldSectionId === (int) $targetSection->id) {
            return back()->with('success', 'المنتج موجود أصلًا داخل هذا القسم.');
        }

        $card->section_id = $targetSection->id;
        // نقل المنتج لقسم جديد يعني أنه لم يعد تابعًا لأي subcategory قديمة.
        $card->subcategory_id = null;
        $card->save();

        return back()->with('success', 'تم نقل المنتج إلى: ' . $targetSection->name);
    }
