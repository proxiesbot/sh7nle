...
class HandleInertiaRequests extends Middleware
{
    ...
    public function share(Request $request): array
    {
        ...
        $shared = array_merge(parent::share($request), [
            'appName' => config('payments.display_name', 'Sh7nle'),
            'flash' => [
                'success' => fn () => $request->session()->get('success'),
                'warning' => fn () => $request->session()->get('warning'),
                'error' => fn () => $request->session()->get('error'),
            ],
            'storeSections' => fn () => Schema::hasTable('sections')
                ? Section::query()
                    ->where('section_id', 0)
                    ->orderBy('name')
                    ->get(['id', 'name', 'icon'])
                : collect(),
            'activeBanner' => fn () => Schema::hasTable('banners')
                ? Banner::query()
                    ...
                    ->first()
                : null,
            'activeBanners' => fn () => Schema::hasTable('banners')
                ? Banner::query()
                    ->where('is_active', true)
                    ->orderBy('sort_order')
                    ->get()
                : collect(),
            'siteSettings' => fn () => Setting::publicSettings(),
            'contentTexts' => fn () => Schema::hasTable('content_texts')
                ? ContentText::query()->pluck('text', 'key')
                : collect(),
            'socialLinks' => fn () => [
                'whatsapp' => Setting::get('social.whatsapp_url', config('payments.social.whatsapp_url')),
                'instagram' => Setting::get('social.instagram_url', config('payments.social.instagram_url')),
                'telegram' => Setting::get('social.telegram_url', config('payments.social.telegram_url')),
                'facebook' => Setting::get('social.facebook_url', config('payments.social.facebook_url')),
        ...
        if ($request->user()) {
            ...
            $shared = array_merge($shared, [
                ...
                'isAdmin' => $isAdmin,
                'creditPrice' => config('payments.credit_price', 1),
            ]);
        }
