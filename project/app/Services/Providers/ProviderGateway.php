...
protected function catalogSwGames(ProviderSource $providerSource, int $parentId = 0): array
{
    $products = collect($this->fetchSwGamesProducts($providerSource))
        ...
        ->map(function ($item) {
            $item = (array) $item;
            $item['__game_id'] = $this->swGamesCategoryId($item);
            $item['__game_name'] = $this->swGamesCategoryName($item);
            $item['__main_group_id'] = $this->swGamesMainGroupId($item);
            $item['__main_group_name'] = $this->swGamesMainGroupName($item);
    ...
    if ($parentId <= 0) {
        $groups = $products
            ->groupBy(fn ($item) => (int) ($item['__main_group_id'] ?? 990006))
            ->map(function ($items, $groupId) {
                $first = (array) $items->first();

                return [
                    'id' => (int) $groupId,
                    'name' => trim((string) ($first['__main_group_name'] ?? 'خدمات أخرى')),
                    'parentId' => 0,
            ...
            ->sortBy(fn ($category) => $this->swGamesMainGroupSort((int) $category['id']))
            ->values()
            ->all();
    ...
    if ($this->isSwGamesMainGroupId((int) $parentId)) {
        $games = $products
            ->filter(fn ($item) => (int) ($item['__main_group_id'] ?? 0) === (int) $parentId)
            ->groupBy(fn ($item) => (int) ($item['__game_id'] ?? 0))
            ->map(function ($items, $gameId) use ($parentId) {
                $first = (array) $items->first();

                return [
                    'id' => (int) $gameId,
                    'name' => trim((string) ($first['__game_name'] ?? 'SW Games')),
    ...
    return [
        ...
        'categories' => [],
        'products' => $products
            ->filter(fn ($item) => (int) ($item['__game_id'] ?? 0) === (int) $parentId)
...
protected function isSwGamesMainGroupId(int $id): bool
{
    return in_array($id, [990001, 990002, 990003, 990004, 990005, 990006], true);
}

protected function swGamesMainGroupSort(int $id): int
{
    return match ($id) {
        990001 => 1,
        990002 => 2,
...
protected function swGamesMainGroupId(array $item): int
{
    return match ($this->swGamesMainGroupName($item)) {
        'ألعاب' => 990001,
        'تطبيقات دردشة' => 990002,
        'بطاقات وحسابات' => 990003,
        'اتصالات' => 990004,
        'إنترنت' => 990005,
        default => 990006,
    };
...
protected function swGamesMainGroupName(array $item): string
{
    $name = mb_strtolower($this->swGamesCategoryName($item) . ' ' . trim((string) ($item['name'] ?? '')));

    foreach (['chat', '4chat', 'ahlan', 'allo', 'azal', 'ayome', 'aswat', 'anghami', 'imo', 'yalla', 'دردشة', 'شات', 'لايف', 'live', 'plus'] as $keyword) {
        if (str_contains($name, mb_strtolower($keyword))) {
            return 'تطبيقات دردشة';
        }
    }

    foreach (['mtn', 'syriatel', 'سيريتل', 'سرياتيل', 'units', 'وحدات', 'رصيد', 'minutes', 'sms'] as $keyword) {
        if (str_contains($name, mb_strtolower($keyword))) {
            return 'اتصالات';
        }
    }

    foreach (['net', 'internet', 'wifi', 'adsl', 'zad', 'زاد', 'انترنت', 'إنترنت', 'mb', 'gb'] as $keyword) {
        if (str_contains($name, mb_strtolower($keyword))) {
            return 'إنترنت';
        }
    }

    foreach (['apple', 'google', 'play', 'steam', 'razer', 'account', 'حساب', 'gift', 'card', 'بطاقة', 'itunes', 'psn', 'xbox', 'amazon', 'netflix', 'spotify'] as $keyword) {
        if (str_contains($name, mb_strtolower($keyword))) {
            return 'بطاقات وحسابات';
        }
    }
...
protected function swGamesCategoryId(array $item): int
{
    $candidates = [
        $item['game_id'] ?? null,
        $item['gameId'] ?? null,
        data_get($item, 'game.id'),
        data_get($item, 'category.id'),
        data_get($item, 'group.id'),
    ];
    ...
    $name = $this->swGamesCategoryName($item);
    $hash = sprintf('%u', crc32('swgames:' . mb_strtolower($name)));
