...
Route::middleware([
    ...
    Route::post('user/transferMoney', [UserController::class, 'transferMoney'])->name('user.transferMoney')->middleware('throttle:10,1');
});

// Deposit status update is admin-only (Critical security fix)
Route::middleware([
    'auth:sanctum',
    config('jetstream.auth_session'),
    'not_blocked',
...
Route::middleware([
    ...
    Route::post('user/{user}/buyCard', [UserController::class, 'buyCard'])->name('user.buyCard')->middleware('throttle:30,1');
    ...
    Route::post('gift-card/redeem', [GiftCardController::class, 'redeem'])->name('giftCards.redeem')->middleware('throttle:10,1');
    Route::get('fortune-wheel', [FortuneWheelController::class, 'index'])->name('wheel.index');
    Route::post('fortune-wheel/spin', [FortuneWheelController::class, 'spin'])->name('wheel.spin')->middleware('throttle:20,1');
    ...
    Route::post('deposits/createPaymentLink', [DepositController::class, 'createPaymentLink'])->name('deposit.createPaymentLink')->middleware('throttle:10,1');
