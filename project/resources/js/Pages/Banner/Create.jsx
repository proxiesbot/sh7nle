...
export default function Create() {
    const { data, setData, post, processing } = useForm({ title: '', subtitle: '', link: '', sortOrder: 0, isActive: 1, image: null });
    const submit = (e) => { e.preventDefault(); post(route('banners.store')); };
    return (
        <AdminLayout title="إضافة بانر">
            <Head title="إضافة بانر" />
            <div className="max-w-3xl mx-auto rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm text-right">
                <form onSubmit={submit} className="space-y-5">
                    ...
                    <div><Label htmlFor="image">الصورة</Label><div className="mt-2 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">المقاس المقترح للبانر: 1600×600 بكسل</div><Input id="image" type="file" accept="image/*" onChange={(e) => setData('image', e.target.files[0])} className="mt-2" /></div>
