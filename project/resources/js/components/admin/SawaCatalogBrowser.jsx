...
export default function SawaCatalogBrowser({ enabled = true, onPickProduct, onImportGroup }) {
    ...
    const loadCatalog = async (parentId = 0, nextTrail = ROOT_TRAIL) => {
        ...
        try {
            ...
            setError(requestError.response?.data?.message || 'تعذر تحميل فهرس Sawa5Card.');
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        if (enabled && !loading && !catalog.categories.length && !catalog.products.length && !error) {
    ...
    return (
        <div className="space-y-4 rounded-3xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-800/40">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <div className="text-sm font-bold text-slate-900 dark:text-white">استعراض مزود Sawa5Card</div>
