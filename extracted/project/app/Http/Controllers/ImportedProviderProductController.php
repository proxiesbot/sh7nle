...
class ImportedProviderProductController extends Controller
{
    ...
    protected function publishImportedProduct(ImportedProviderProduct $importedProduct, array $validated): array
    {
        ...
        $isRootTarget = ! empty($validated['root']);

        if (! $section && ! $isRootTarget) {
        ...
        // Find an existing provider product globally first, not only inside the
        // target section. This lets a corrected SW Games path move the old card
        // from a wrong auto-created section such as "تطبيقات مباشرة" instead of
    ...
    protected function normalizeProductTrail(array $trail, array $normalizedProduct, array $fallbackProduct = []): array
    {
        ...
        $categoryCandidates = collect([
            $normalizedProduct['categoryNames'] ?? null,
            $normalizedProduct['category_names'] ?? null,
            $normalizedProduct['categoryPath'] ?? null,
            $normalizedProduct['category_path'] ?? null,
            $normalizedProduct['categoryName'] ?? null,
            $fallbackProduct['__sw_location_trail'] ?? null,
            $fallbackProduct['categoryNames'] ?? null,
            $fallbackProduct['category_names'] ?? null,
            $fallbackProduct['categoryPath'] ?? null,
            $fallbackProduct['category_path'] ?? null,
