...
class DepositController extends Controller
{
    ...
    public function create(Request $request): Response
    {
        return Inertia::render('Deposit/Create', [
            ...
            'usdRate' => (float) Setting::get('payments.deposit_usd_to_syp_rate', env('CREDIT_PRICE', 1)),
    ...
    public function createPaymentLink(CreatePaymentLinkRequest $request): JsonResponse
    {
        ...
        if ($currency === 'SYP') {
            $amount *= (float) env('CREDIT_PRICE', 1);
        }

        $response = Http::withHeaders([
            'x-api-key' => env('KAZAWALLET_API_KEY'),
    ...
    {
        return $user->id . env('KAZAWALLET_REF_TOKEN');
    }

    public function getUserIdFromPaymentRef(string $paymentRef): ?int
    {
        $token = env('KAZAWALLET_REF_TOKEN');
    ...
    {
        $kazawalletApiKey = env('KAZAWALLET_API_KEY');
        $kazawalletApiSecret = env('KAZAWALLET_SECRET');
    ...
    {
        if ($currency === 'SYP') {
            $creditPrice = (float) env('CREDIT_PRICE', 1);
    ...
    protected function getOrCreateKazawalletMethod(): PaymentMethod
    {
        return PaymentMethod::query()->firstOrCreate(
            ...
            [
                ...
                'account' => env('KAZAWALLET_EMAIL', 'Automatic payment link'),
